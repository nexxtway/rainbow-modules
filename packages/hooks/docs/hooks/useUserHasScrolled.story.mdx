...
`useUserHasScrolled` A hook that elegantly manages window scrolling.

### API

```ts
const userHasScrolled = useUserHasScrolled({});
```
...
| triggerFn | the trigger function to calculate scroll intention               | function |         |

you can also pass aditional options that will be passed to triggerFn

`triggerFn` will receive `target`, `currentScrollPos`, and `aditionalOptions` as params
