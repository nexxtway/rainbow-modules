...
import { WrapperScrollingIntent } from '../stories';

<Meta title="Modules/Hooks/useScrollingIntent" />

# Overview

`useScrollingIntent` A hook to detect user scroll intention.

### API

```ts
useScrollingIntent({ callback, isListening, triggerElementRef, threshold });
```
...
| ----------------- | ----------------------------------------------- | --------------- | ---------------- | --- |
| callback          | Callback function that trigger on scroll intent | (event) => void |                  |
| isListening       | Add or remove the event listener                | boolean         | true             |
| triggerElementRef | Dom ref of the trigger element                  | ElementRef      | () => ElementRef |     |
| threshold         | Limit in px that will trigger the callback      | number          | 20               |
