...
`useAuthFetch` hook will return an object with a `fetchAsync` function and a `isLoading` state.
The fetchAsync function will use the native fetch API to make a request to a firebase https function.
